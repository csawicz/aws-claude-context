...
export class Context {
    ...
    constructor(config: ContextConfig = {}) {
        // Initialize embedding service - default to AWS Bedrock if AWS credentials are available
        if (config.embedding) {
            this.embedding = config.embedding;
        } else if (envManager.get('AWS_REGION') || envManager.get('AWS_ACCESS_KEY_ID')) {
            // Use AWS Bedrock by default when AWS credentials are available
            this.embedding = new BedrockEmbedding({
                region: envManager.get('AWS_REGION'),
                accessKeyId: envManager.get('AWS_ACCESS_KEY_ID'),
                secretAccessKey: envManager.get('AWS_SECRET_ACCESS_KEY'),
                sessionToken: envManager.get('AWS_SESSION_TOKEN'),
                model: envManager.get('BEDROCK_EMBEDDING_MODEL') || 'amazon.titan-embed-text-v2:0'
            });
        } else {
            // Fallback to OpenAI
        ...
        // Initialize vector database - default to S3 if S3 bucket is configured
        if (config.vectorDatabase) {
            this.vectorDatabase = config.vectorDatabase;
        } else if (envManager.get('S3_VECTORS_BUCKET_NAME')) {
            // Use S3Vectors by default when S3Vectors bucket is configured
            this.vectorDatabase = new S3VectorsDatabase({
                bucketName: envManager.get('S3_VECTORS_BUCKET_NAME')!,
                region: envManager.get('AWS_REGION'),
                accessKeyId: envManager.get('AWS_ACCESS_KEY_ID'),
                secretAccessKey: envManager.get('AWS_SECRET_ACCESS_KEY'),
                sessionToken: envManager.get('AWS_SESSION_TOKEN')
            ...
            throw new Error('VectorDatabase is required. Please provide a vectorDatabase instance in the config or set S3_VECTORS_BUCKET_NAME environment variable.');
        }
