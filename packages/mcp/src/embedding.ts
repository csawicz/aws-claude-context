    ...
    switch (config.embeddingProvider) {
        ...
        case 'Bedrock':
            if (!config.awsRegion && !config.awsAccessKeyId) {
                console.error(`[EMBEDDING] ❌ AWS credentials or region required for Bedrock embedding provider`);
                throw new Error('AWS_REGION or AWS credentials are required for Bedrock embedding provider');
            }
            console.log(`[EMBEDDING] 🔧 Configuring AWS Bedrock with model: ${config.embeddingModel}, region: ${config.awsRegion || 'us-east-1'}`);
            ...
            console.log(`[EMBEDDING] ✅ AWS Bedrock embedding instance created successfully`);
...
export function logEmbeddingProviderInfo(config: ContextMcpConfig, embedding: OpenAIEmbedding | VoyageAIEmbedding | GeminiEmbedding | OllamaEmbedding | BedrockEmbedding): void {
    ...
    switch (config.embeddingProvider) {
        ...
        case 'Ollama':
            ...
            break;
        case 'Bedrock':
            console.log(`[EMBEDDING] Bedrock configuration - Region: ${config.awsRegion || 'us-east-1'}, Model: ${config.embeddingModel}`);
            console.log(`[EMBEDDING] AWS Credentials: ${config.awsAccessKeyId ? '✅ Provided' : '❌ Missing'}`);
            break;
    }
} 