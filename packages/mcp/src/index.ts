...
import { MilvusVectorDatabase, S3VectorsDatabase } from "@zilliz/claude-context-core";
    ...
    constructor(config: ContextMcpConfig) {
        ...
        if (config.vectorDatabase === 'S3Vectors') {
            if (!config.s3VectorsBucketName) {
                throw new Error('S3_VECTORS_BUCKET_NAME is required when using S3Vectors vector database');
            }
            console.log(`[VECTOR-DB] Initializing S3Vectors database with bucket: ${config.s3VectorsBucketName}`);
            vectorDatabase = new S3VectorsDatabase({
                bucketName: config.s3VectorsBucketName,
            ...
            console.log(`[VECTOR-DB] Initializing Milvus database`);
